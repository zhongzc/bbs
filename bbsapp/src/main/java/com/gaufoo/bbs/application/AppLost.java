...
public class AppLost {
    ...
    private static Procedure<ErrorCode, LostId> publishLostPost(Lost.LostInput input, UserId publisher, FileId nullablePictureId) {
        return Procedure.fromOptional(componentFactory.lost.publishPost(
                LostInfo.of(input.itemName, publisher.value, input.description, input.position, nilOrTr(nullablePictureId, x -> x.value),
    ...
    private static Lost.LostInfo consLostInfo(Lost.LostInput input, LostId id, UserId publisherId, FileId nullableFileId) {
        return new Lost.LostInfo() {
